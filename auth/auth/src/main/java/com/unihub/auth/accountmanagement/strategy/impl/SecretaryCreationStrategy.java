...
import com.unihub.auth.accountmanagement.dto.request.SecretaryUserRequest;
...
public class SecretaryCreationStrategy implements UserCreationStrategy {
    ...
    public UserDto createUser(BaseUserRequest userRequest, Authentication authentication) {
        ...
        user.setUniversityMetadata(
                UniversityMetadata.builder()
                        ...
                        .cid(((SecretaryUserRequest)userRequest).getCid())
                        .user(user)
                        .build()
        );
    ...
    public UserRoles getSupportedRole(){
        return UserRoles.ROLE_SECRETARY;
    }
